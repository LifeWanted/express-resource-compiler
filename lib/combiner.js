...
function combine( opts, sourceFiles, destinationFile, callback ){
    if( opts.stream ){
        ...
        var compilerStream      = opts.compile( destinationStream, sourceFiles[ 0 ] );
        ...
        async.waterfall([
            async.forEach.bind( async, sourceFiles, readFile ),
            combineFiles,
            opts.compiler.bind( opts, sourceFiles[ 0 ] ),
...
module.exports = function( compiler, opts ){
    ...
    return function( req, res, next ){
        ...
        var sourceDirectory = path.join( sourcePath         || req.route.path, noExt );
        var destinationFile = path.join( destinationPath    || req.route.path, path.basename( file ) );
